...
\end{itemize}
Naast elliptische krommen kan ook iets anders worden  gekozen. Zolang het iets is waarop je een bewerking kan doen die moeilijk om te keren is en dat het een commutatieve bewerking is.\\
...
\item De punten van de grafiek: $E$ moet beschreven kunnen worden met een bijectieve parametrisatie, oftwel $\exists p \in C(R\to E)$ bijectief.
...
we trekken een lijn door $A$ en $B$, laat $C$ het andere snijpunt met de grafiek zijn. We spiegelen $C$ in de $x$-as en het resultaat is $A+B$.(VOEG PLAATJE TOE HIERVOOR)\\
Hier zitten uiteraard nog haken en ogen aan. Hoe gaan we om met optellen met $\infty$? Is er wel altijd precies  \'{e}\'{e}n ander snijpunt? Aangezien we $\infty$ als eenheidselement hebben gedefini\'{e}erd, nemen we het punt waarbij $\infty$ wordt opgeteld als resultaat. Vervolgens als we een punt $A$ bij zijn $x$-as spiegeling optellen defini\"eren we dat daar $\infty$ uit komt. Als gevolg hiervan noemen we de $x$-as spiegeling van $A$, $-A$.\\
Als laatste wat we moeten laten zien is dat uit gezonderd van $A + -A$ dat er in het algoritme er daadwerkelijk een derde snijpunt is. (VOEG HIER DIE BESCHRIJVING VAN DE WEBPAGINA TOE)
\newpage
\subsection{Eindig maken}
Zoals we eerder vermeld hadden willen we een \emph{eindige} groep hebben, dat is de groep hierboven besproken niet of hij bevat alleen $\infty$ wat ook niet nuttig is. De manier waarop we dat doen is de waarden van $x$ en $y$ beperken tot een eindig veld, waarbij alleen gehele getallen zitten. Een eindig veld is een verzameling met daarbij een multiplicatie, additie, subtractie en divisie.\\
