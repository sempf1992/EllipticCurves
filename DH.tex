...
\\Als we gaan kijken hoeveel energie het kost om AES met 256 bit sleutels te ontsleutelen merken we het volgende op. We kunnen per bitflit berekenen hoeveel het kost om deze uit te voeren. Deze is van beneden begrensd door Landauers princiepe \cite{Landau}. Deze zegt dat het minimaal $kT ln(2)$ energie kost per berekening, waar $k$ de boltzman constante is, $T$ de temperatuur in kelvin is en $ln(2)$ het natuurlijke algoritme van $2$. Nu kunnen we een berekenen wat het zou kosten om een computer op $1$ kelvin te laten bereken.
Als we er van uit gaan dat het uitvoeren van een bitflip $5 \times 10^{-5}$ electronvolt kost, en we de volledige energie productie van de zon kunnen gebruiken om bitflips te genereren kost het meer dan een jaar aan productie van de zon om $2^{256}$ bitflips aan energie te produceren. Dit is de kleinste energiegrootte die gebruikt kan worden om berekeningen te doen. Een bericht dat versleuteld is met een 256 bit sleutel zal dan ook gegarandeerd een jaar veilig zijn, zolang we niet nog meer dan alleen de volledige energieproductie van de zon kunnen benutten.
...
Bijna al onze methodes zijn $O(1)$. Uitzonderingen daarop zijn bijvoorbeeld de methodes die de berichten verwerken, deze zijn lineair in de grootte van het bericht. De methode om punten te vermenigvuldigen met scalars wordt uitgevoerd in $O(\log(\text{scalar}))$, zo ook de machtsverheffing van FiniteFieldElem's $O(\log(\text{macht})$. Dit heeft als gevolg dat in de key-exchange stappen $O(\log(\text{macht})$. De methoden om berichten te versturen zijn natuurlijk $O(n)$. 
\section{Beperkingen}
Er zijn enkele beperkingen van onze code. Zo gaat python, afhankelijk van het besturingssysteem, anders om met select. Op windows werkt onze code daarom niet omdat we select alleen mogen toepassen op sockets. Op $*nix$ systemen gaat het wel goed, want daar werkt select op alles dat een readmethode heeft. Op $*nix$ systemen zie je artefacten hoe stdin en stdout in de console werken. Dit heeft bijvoorbeeld tot gevolg dat als je op commandotoetsen drukt de ascii notatie verschijnt. Verder als er nieuwe stdout wordt gegeven, wordt de input voor stdin onzichtbaar. Dat wil echter niet zeggen dat deze er niet meer is, de console heeft het gewoon onzichtbaar gemaakt. "Onverwachts verbinding verbreken en meteen een nieuwe aanmaken" kan ervoor zorgen dat de Broken pipe-error wordt geraised. Dit komt doordat de lijst met open sockets al een socket bevat met dat adres en poort. Dit wordt herkent en als fout doorgegeven, ook als het correct zou zijn.
...
\bibitem{Landau} https://en.wikipedia.org/wiki/Landauer\%27s\_principle
\end{thebibliography}
